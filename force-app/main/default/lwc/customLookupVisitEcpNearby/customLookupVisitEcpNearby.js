   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    connectedCallback(){
        ...
        if(this.defaultRecordId != ''){
            searchFirstCompetitorDefault({ recordId: this.defaultRecordId , 'sObjectApiName' : this.sObjectApiName })
            .then((result) => {
                if(result != null){
    ...
    @wire(searchFirstCompetitor, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName', recordId : '$receivedId'})
     searchResult(value) {
